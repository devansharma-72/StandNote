...
div.innerHTML = `
  ...
  <div class="standnote-action-buttons">
    <img id="stopBtn" title="Generate MoM" src=${chrome.runtime.getURL(
      './assets/done.png'
    )} />
    <img id="pauseBtn" title="Pause" src=${chrome.runtime.getURL(
      './assets/paused.png'
    )} />
    <img id="muteBtn" title="Mute microphone" src=${chrome.runtime.getURL(
      './assets/mic-play.png'
    )} />
  </div>
`;
...
function setToDefaultAndInject() {
  ...
  document.getElementById('pauseBtn').src = chrome.runtime.getURL(
    './assets/paused.png'
  );

  document.getElementById('muteBtn').title = 'Mute microphone';
  document.getElementById('muteBtn').src = chrome.runtime.getURL(
    './assets/mic-play.png'
  );
...
function pause() {
  ...
  if (!paused) {
    stopTimer();
    pauseBtn.title = 'Play';
    pauseBtn.src = chrome.runtime.getURL('./assets/play.png');
    paused = true;
  } else {
    startTimer();
    pauseBtn.title = 'Pause';
    pauseBtn.src = chrome.runtime.getURL('./assets/paused.png');
    paused = false;
  }
}
...
function muteMic() {
  ...
  if (!muted) {
    ...
    muteBtn.src = chrome.runtime.getURL('./assets/mic-paused.png');
    muted = true;
  } else {
    muteBtn.title = 'Mute microphone';
    muteBtn.src = chrome.runtime.getURL('./assets/mic-play.png');
    muted = false;
  }
}
